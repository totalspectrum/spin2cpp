...
(2) Data may not be declared or used. That is, a line like `myvar long 0` is not legal in FlexSpin inline assembly. Instead, declare a local variable `myvar` and initialize it to 0.
...
No address may be given in an ORG/END pair. In flexspin inline assembly is run from LUT rather than from COG, using the FCACHE mechanism. If no FCACHE is available (e.g. -O0 is given) then the code is run as hubexec. This means that data may not declared in inline assembly. That is, instead of writing:
```
myVar long 0
myVar2 long 2
```
you must declare local variables `myVar` and `myVar2` before the inline assembly, and initialize them to `0` and `2` respectively.
