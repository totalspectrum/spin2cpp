...
PUB waitcnt2(newcnt, incr)
  ...
  return newcnt
```
There is one important difference between `org`/`end` and `asm`/`endasm`. Normally inline assembly is treated the same as code generated by the compiler, and subject to optimization. However, code between `org` and `end` is not optimized. So if your inline assembly is timing sensitive, you should use `org` rather than `asm` to start it.

Also note that unlike the "official" Spin2 compiler, fastspin does not accept an address for the inline assembly, and the assembly is not called (so no `ret` statement should be included).
