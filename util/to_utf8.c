...
/* convert a wide character wcorig to a sequence of
 * UTF-8 bytes stored in s; returns the count
 * WARNING: s must be large enough to hold any sequence
 * (8 bytes should do)
 */
size_t
to_utf8(char *s, wchar_t wcorig)
{
  uint32_t wc = wcorig;
  uint32_t count;
  uint32_t left;
